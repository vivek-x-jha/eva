...
# build and preview the main man page (eva.1)
[group('documentation')]
@man-1-preview: man
    man "${CARGO_TARGET_DIR:-target}/man/eva.1"

# build and preview the colour configuration man page (eva_colors.5)
[group('documentation')]
@man-5-preview: man
    man "${CARGO_TARGET_DIR:-target}/man/eva_colors.5"

# build and preview the colour configuration man page (eva_colors-explanation.5)
[group('documentation')]
@man-5-explanations-preview: man
    man "${CARGO_TARGET_DIR:-target}/man/eva_colors-explanation.5"
...
release:
    ...
    git commit -asm "release(eva): release {{new_version}}"
    git push -u origin HEAD
    @echo "waiting 10 seconds for github to catch up..."
    sleep 10
    gh pr create --draft --title "release(eva): release {{new_version}}" --body "Automated Eva release preparation."
    @echo "Now go review that and come back and run gh-release"

[group('release')]
@gh-release:
    git tag -d "v{{new_version}}" || echo "tag not found, creating";
    git tag --sign -a "v{{new_version}}" -m "auto generated by the justfile for eva v$(convco version)"
    just cross
    just mangen
    just completions
    ...
    gh release create "v$(convco version)" --target "$(git rev-parse HEAD)" --title "eva v$(convco version)" -d -F ./target/"release-notes-$(convco version)/RELEASE.md" ./target/"bin-$(convco version)"/* ./target/"man-$(convco version).tar.gz" ./target/"completions-$(convco version).tar.gz"
...
# Generate release binaries for eva
#
# usage: cross
[group('binaries')]
@cross:
    ...
    ## Linux
    ### x86
    just binary eva x86_64-unknown-linux-gnu
    # just binary_static eva x86_64-unknown-linux-gnu
    just binary eva x86_64-unknown-linux-musl
    # just binary_static eva x86_64-unknown-linux-musl

    ### aarch
    just binary eva aarch64-unknown-linux-gnu
    just binary_no_libgit eva aarch64-unknown-linux-gnu
    # BUG: just binary_static eva aarch64-unknown-linux-gnu

    ### arm
    just binary eva arm-unknown-linux-gnueabihf
    just binary_no_libgit eva arm-unknown-linux-gnueabihf
    # just binary_static eva arm-unknown-linux-gnueabihf

    ## MacOS
    # TODO: just binary eva x86_64-apple-darwin

    ## Windows
    ### x86
    just binary eva.exe x86_64-pc-windows-gnu
    # just binary_static eva.exe x86_64-pc-windows-gnu
    # TODO: just binary eva.exe x86_64-pc-windows-gnullvm
    # TODO: just binary eva.exe x86_64-pc-windows-msvc
...
@mangen:
    ...
    pandoc --standalone -f markdown -t man man/eva.1.md > ./target/"man-$(convco version)"/eva.1
    pandoc --standalone -f markdown -t man man/eva_colors.5.md > ./target/"man-$(convco version)"/eva_colors.5
    pandoc --standalone -f markdown -t man man/eva_colors-explanation.5.md > ./target/"man-$(convco version)"/eva_colors-explanation.5
